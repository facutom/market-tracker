...
import feedparser
import pytz
from streamlit_autorefresh import st_autorefresh # Asegúrate de que esté en requirements.txt

# 1. REFRESCAR CADA 30 SEGUNDOS (OBLIGATORIO AL PRINCIPIO)
st_autorefresh(interval=30000, limit=None, key="nasdaq_auto_update")
...
# CSS (Con animación pulsante para el punto verde)
st.markdown(f"""
<style>
...
.status-tag {{ display: inline-flex; align-items: center; gap: 8px; font-size: 0.75rem; font-weight: 700; margin-top: 5px; }}
...
def get_market_status():
    ...
    # Reloj visible para comprobar el refresco
    clock_str = now.strftime("%H:%M:%S")
    if is_weekday and (m_open <= now.time() <= m_close):
        return f"LIVE ({clock_str} NY)", "dot-live", "#00ff41", now.date()
    return f"CLOSED ({clock_str} NY)", "dot-closed", "#787b86", now.date()
...
if not df.empty:
    ...
    val_real = live_price if ("LIVE" in status_text and live_price) else last_yesterday["Precio Real"]
    ...
    df_fut = df[df["Fecha"].dt.date >= target_date]
    one_year_target = df_fut.iloc[0]["Precio Sintético"] if not df_fut.empty else 0
    ...
    if news:
        for item in news:
            st.markdown(f"""<div class="news-item"><div class="news-date" style="color:#787b86; font-size:0.8rem;">{item['date']}</div><div class="news-text">{item['title']}</div><a href="{item['link']}" style="color:#2962ff; font-size:0.85rem; text-decoration:none;" target="_blank">Read full article →</a></div>""", unsafe_allow_html=True)
